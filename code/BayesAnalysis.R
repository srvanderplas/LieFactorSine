...
#   temp <- sum(dnorm(data$ans.liefactor, mean=par[1], sd=par[2], log=TRUE))
...
#--------------------Overall Marginals-------------------------------------------------------------------------
# pars <- as.matrix(expand.grid(seq(0, 3, .025), seq(.1, .5, .01)))
# 
...
#   xlab("Mean Psychological Lie Factor") + ylab("Std. Deviation") + xlim(c(1, 1.75))
...
# 
# pars <- as.matrix(expand.grid(seq(0, 3, .025), seq(.1, .5, .025)))
# 
...
ggplot() + 
  ...
  facet_grid(.~functions) + ylab("Density") + xlab("Lie Factor") + theme_bw() + scale_colour_discrete("Function Type") +
