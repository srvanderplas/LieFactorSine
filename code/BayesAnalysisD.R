...
logpost <- function(data, par){
  ...
  temp <- sum(dnorm(data$ans.d, mean=par[1], sd=par[2], log=TRUE))
...
ggsave("figure/fig-Joint2dDensityD.pdf", width=4, height=4, units="in")

#--------------------Individual Distribution of Theta-------------------------------------------------------------------------

pars <- as.matrix(expand.grid(seq(.5, 2.5, .01), seq(.1, .5, .025)))
...
ggsave("figure/fig-IndivMeanAllFcnsD.pdf", width=7, height=3.5)      
...
ggsave("figure/fig-CIindivMeanD.pdf", width=6, height=6, units="in")
...
ggsave("figure/fig-spaghettiIndivDistsD.pdf", width=6, height=6, units="in")
...
save.image("./code/BayesAnalysisD.Rdata")