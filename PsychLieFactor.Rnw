...
w \cdot \cos x,
\]
where $w$ is varied between 0 (for no correction) and 1.5 (very strong correction). From the previous pilot study, we suspected values of $w = 0.8$ to be promising. This led to twelve sheets with correction factors as shown in table \ref{tab:w}. The difficulty level of each of the sheets was determined by the similarity of the six plots shown.
...
As the strength of the correction varies across the horizontal range of the curve, we  quantify the psychological distortion by comparing the maximum line length to the minimum line length for each plot $i$: $D^i = (l_{max}-l_{min})/l_{min}$. 
...
%
By averaging a participant's preference for the plot with the most consistent line length, we can obtain an estimate of the psychological distortion from the sine illusion. 
