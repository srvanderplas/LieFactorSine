...
XXX How was the amount of correction picked for the study?
XXX difficulty level



Besides the sine wave two other curves were tested, as shown in figure \ref{fig:stimuli}. Figure \ref{fig:stimuli.a} whos an inverse function ($f(x) \propto 1/x$),  figure \ref{fig:stimuli.b} shows line lengths along an exponential ($f(x) \propto \exp(a*x)$). Note that for both figures the line lengths are kept constant.
...
\end{subfigure}
\caption{\label{fig:stimuli} Vertical line segments along two curves. Line segments are kept constant but appear to be of different length due to a strong contextual illusion.}
\end{figure}

five sheets with the sine wave as stimuli of difficulty 1 through 5. 
five sheets with either the exponential curve or the inverse curve as stimuli of difficulty 1 through 5. 
one test plot 
