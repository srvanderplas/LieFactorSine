...
The overall distribution of $\theta_i$ and $\theta$ for each stimulus type is shown in Figure \ref{fig:overalldensity}. Estimates of $\hat\theta = E[\theta]$ for each function are \Sexpr{overall.mean.bounds$mean[1]}, \Sexpr{overall.mean.bounds$mean[2]}, and \Sexpr{overall.mean.bounds$mean[3]} respectively for exponential, inverse, and sine functions, suggesting a similar psychological distortion even for very different functions. 
...
overall.mean.bounds
keys <- unique(data.frame(lie=key$lmax*2, w=key$w, test=key$test_param))
...
orig <- createSine(50, 1, f, fprime, f2prime, 0, 2*pi)
lims <- getYlim(c(0, 1.4), orig, f, fprime, f2prime)
w <-  c(0, midweights$w[3], 1)
frameorder <- w

w.label <-  function(var, value){
  value <- paste("w = ", round(w[value], 2))
  return(value)
}
...
data$set <- sapply(data$w, function(i) which(w %in% i))
data$display <- sapply(data$w, function(i) which(frameorder %in% i))
qplot(data=data, x=xstart, y=ystart, xend=xend, yend=yend, geom="segment") + facet_grid(.~display, labeller=w.label) + coord_equal(ratio=1) + theme_stimuli() + xlim(lims$dx) + ylim(lims$dy)
ggsave("figure/fig-sineOptimalWeights.pdf", width=6, height=2.25)
...
data$display <- sapply(data$w, function(i) which(frameorder %in% i))
qplot(data=data, x=xstart, y=ystart, xend=xend, yend=yend, geom="segment") + facet_grid(.~display, labeller=w.label) + coord_equal(ratio=1) + theme_stimuli() + xlim(lims$dx) + ylim(lims$dy)
ggsave("figure/fig-invOptimalWeights.pdf", width=6, height=2.25)
...
data <- rbind.fill(ldply(w, function(i) weightYTrans(orig, i)))
data$set <- sapply(data$w, function(i) which(w %in% i))
data$display <- sapply(data$w, function(i) which(frameorder %in% i)+2)
qplot(data=data, x=xstart, y=ystart, xend=xend, yend=yend, geom="segment") + facet_grid(.~display, labeller=w.label) + coord_equal(ratio=1) + theme_stimuli() + xlim(lims$dx) + ylim(lims$dy)
ggsave("figure/fig-expOptimalWeights.pdf", width=6, height=2.25)
...
@
<<preferredstimuli,echo=FALSE,include=FALSE>>=
