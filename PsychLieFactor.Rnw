...
The three-dimensional context most similar to the sine illusion shown in Figure \ref{fig:sineillusion} is shown in Figure \ref{fig:sineillusion3d}. 
\begin{figure}[h!t]\centering
...
p <- persp(x, z, data.persp, ylab="z", zlab="y", theta=-5, phi=25, border="black", shade=.35, col="white", xlim=c(-pi/12, 2*pi+pi/12), ylim=c(-1.75, 1.75), expand=.75, scale=FALSE) # , ltheta=0, lphi=-15
# lines(trans3d(x, -1.5, 2*sin(x), p), col="white")
# lines(trans3d(x, 1.5, 2*sin(x), p), col="white")
