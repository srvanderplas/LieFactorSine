...
In order to determine the size of the psychological distortion, participants were presented with 10 sets of 6 graphs similar to Figure \ref{fig:sinestimuli}. These sets were constructed so that the line lengths of some of the sub-plots varied to counteract the illusion. Participants were asked "In which graph is the size of the curve most consistent?," which was chosen so as not to bias participants to explicitly measure line lengths. Participants were also asked to provide an estimate of their confidence in their answer, rated on a 5 point likert scale from low to high. The actual differences in line length corresponding to Figure \ref{fig:sinestimuli} are shown in Figure \ref{fig:sinestimulinotrend}. In addition to 6 sets of 6 plots with varying difficulty and line length corrections such as in Figure \ref{fig:sinestimuli}, participants were also shown 5 sets of 6 plots with either an inverse ($f(x) \propto 1/x$) or exponential ($f(x) \propto \exp(a*x)$) curve. Plots were shown to participants in random order, and were ranked by difficulty so that each participant received one plot of each of 5 levels of difficulty (as measured by the similarity of the 6 plots shown). After the 11 plots required to receive payment through Amazon, participants were shown randomly selected plots from the 32 possible stimuli. In order to more accurately identify unique individuals, data were grouped by IP address rather than by user-entered identification code, as it is common for individuals to have multiple accounts and somewhat less likely for two Amazon Mechanical Turk participants to share the same IP address and complete the same study in the same reasonably short window. 

\begin{figure*}
\center
\begin{subfigure}[b]{.9\linewidth}\centering
\includegraphics{figure/fig-stimulicorrectionsize1}
\caption{One of the graphs presented to participants through Amazon Mechanical Turk.}\label{fig:sinestimuli}
\end{subfigure}
\begin{subfigure}[b]{.9\linewidth}\centering
\includegraphics{figure/fig-stimulicorrectionsize2}
