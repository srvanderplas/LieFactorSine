...
\]
where $w$ is varied between 0 (for no correction) and 1.5 (very strong correction). From a pilot study, we suspected values around $w = 0.8$ to be promising, but did not know whether this would generalize to those outside of the statistical graphics community. This led to twelve stimuli sets with correction factors as shown in table~\ref{tab:w}. The difficulty level of each of the twelve stimuli sets was determined by the similarity of the six plots shown.

\begin{figure}
\centering
\includegraphics[width=6in, trim=.5in 0in .5in 0in ]{figure/fig-stimulicorrectionsize1}
...
\begin{figure}
\centering
\includegraphics[width=6in, trim=.5in .15in .5in 0in ]{images/fig-stimulicorrectionsize2}
...
\begin{table}[ht]
\centering
\tbl{\label{tab:w} Weight factors used to create the sine curve stimuli.}{
...
for each plot and each participant. That is, $P$ is the ratio of the lie factor of the chosen plot to the smallest lie factor available in the set of available plots. When the set of available plots contains an uncorrected plot, $P = D$; when an uncorrected plot is not presented, $P < D$, since $D_j >= 1$. This transformation is a conservative approach to estimating the lie factor, but allows us to show a variety of scaled transformations and estimate the effect with more granularity around $w=0.8$. 
...
\tbl{Credible intervals for the overall $\theta$ for exponential, inverse, and sine stimuli.\label{tab:credibleintervalstable}}{\begin{tabular}{ccc}
...
\caption{Uncorrected, optimally corrected (according to the mean value of $\theta$), and fully corrected stimuli for all three underlying functions used in the experiment. The corrected value shown here is equivalent to the distortion factor $D$ defined as $D=\ell_{max}/\ell_{min}$.}\label{fig:allCorrectionOptimal}
\end{figure}
