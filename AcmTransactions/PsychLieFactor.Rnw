...
While the classic M\"uller-Lyer illusion is seldom a factor in statistical graphics, there are other illusions caused by the interpretation of a two-dimensional stimulus in the context of three-dimensional objects, leading to a distortion in the mental representation of the original stimulus. The sine illusion (also known as the line width illusion \hh{XXX cite Day and stecher, and ours}) is one example of this phenomenon which occurs frequently in statistical graphics. Like the M\"uller-Lyer illusion, it is pervasive and very difficult to ``un-see" or mentally correct. \hh{XXX what is the sine illusion in words?} Figure~\ref{fig:sineillusion} shows the sine illusion in its original form, and figure~\ref{fig:playfair} shows the same illusion in one of the charts from Playfair's Statistical Atlas~\cite{playfair}. This illusion, which is also known as the line-width illusion, has also been documented in parallel sets plots \cite{parallelsets}. 
...
The psychological mechanisms which force three-dimensional context onto two-dimensional stimuli are useful adaptations to a three-dimensional world~\cite{gregory}, but they do have disadvantages when applied to abstract two-dimensional stimuli, such as statistical graphics. In order to estimate the psychological lie factor that occurs due to this illusion, we assessed the strength of the illusion experimentally. 
\hh{XXX define the psychological lie factor somewhere.}
...
\]
where $w$ is varied between 0 (for no correction) and 1.5 (strong over-correction) - \hh{XXX ah, slow down! that's two aspects in one sentence. $w$ is a weighting factor - which gives 0 and 1 special meaning. the choice of 1.5 is already a design choice for the experiment - that needs a separate sentence}. From a pilot study, we suspected values around $w = 0.8$ to be promising \hh{XXX promising for what?}, but did not know whether this would generalize to those outside of the statistical graphics community. \hh{XXX sorry, I'm not following - how did this lead to twelve sets? Do you mean that we picked twelve sets of stimuli? - describe the process for picking a set. } This led to twelve stimuli sets with correction factors as shown in table~\ref{tab:w}. \hh{XXX rephrase caption text and repeat in the writeup.}
...
\caption{One of the graphs presented to participants through Amazon Mechanical Turk. Figure \ref{fig:sinestimulinotrend} shows the actual differences in line lengths. \hh{XXX which of the weights in table~\ref{tab:w} correspond to this chart?} 
...
Plots were shown to participants in random order. The test chart consisted of a set of six sine curves with a very low level difficulty level, and is used as an introduction to the testing procedure. \hh{XXX this contradicts the random order - describe the test setting in a bit more detail, that'll clear up the confusion.}
