...
% \comment{The lie factor needs more details for a proper introduction, like the one you do a bit later. Take out some detail and just mention the lie factor as one measure to assess whether a picture is true to the data it's supposed to show.}
...
c_\ell(w) = (1-w) + w \sqrt{\left(1+f^\prime(x)^2\right)}
\end{equation}
...
\newdo{ I think we should first explain that we estimate the psychological lie factor from the values of the chosen $D_k$.}

\newtext{In the experiment, we ask onlookers to identify the subplot from the chart with the most consistent lines along the function $f(x)$. This, in turn, identifies the corresponding $D_k$ value as the one which leads to the least amount of perceived distortion, which allows us to estimate the psychological lie factor underlying function $f(x)$. }
...
\newtext{Note that $D_k$ and the weight used for the correction factor $w$ depend on each other (see equation (\ref{eqn:correction})). 
}
...
\newdo{Could we please call $D_k$ and $D_{ijk}$ just `distortion' at the moment? It turns into the psychological lie factor once participants pick it, but we shouldn't be calling it psychological factor beforehand.}

\newtext{We quantify the amount of distortion across the horizontal range of the curve as the }
%As the strength of the correction varies across the horizontal range of the curve, we quantify the psychological distortion as the 
ratio of the maximum line length to the minimum line length for each sub-plot $k$: $D_k = l_\text{max}/l_\text{min}$ (see definition (\ref{eqn:psychliefactorD})). 

%For a given set of 6 sub-plots, $j$, $D_{jk}$ would denote the sub-plot distortion factor. 
...
% \hh{XXX is $l_\text{min}$ always equal to 1 in our experiment?}

% \hh{XXX I think $D^\ast$ needs a bit more formality. 
% Let's call it $D^\ast_{ij}$ for participant's $i$ choice on the $j$th set. You have 36? different sets, so $1 \le j \le 36$ and $1 \le i \le 128$. For each participant you have at least 11 responses, more for some.  It might be beneficial to even have a third subscript $k$ and introduce this as $D^\ast_{ijk}$ for $k \in \{1,2,3\}$, $1 \le j \le 12$ and $1 \le i \le 128$.}


% \hh{XXX why only extension?  - That was just our choice. We could also have worked the correction by averaging across lines to keep the average at 1.}

The correction for the sine illusion we use here extends the line segments, so that for initial line segments of length 1, the correction  produces  line segments of length greater than or equal to 1. For all of the functions in the study, 
%In addition, due to the underlying functions we have chosen, 
the minimum line length (assuming a starting line length of 1) after correction is approximately 1; this allows us to simplify $D_k$ as $$D_k = \text{maximum line length in plot } k$$
...
%That is, $P$ is the ratio of the lie factor of the chosen plot to the smallest lie factor available in the set of available plots. 
Whenever the set of available plots contains an uncorrected plot, the psycholgical lie factor $P$ is equal to $D$; otherwise, i.e.\ no uncorrected plot is  presented, the psycholgical lie factor $P$ is less than the distortion $D$. %, since each $D_j >= 1$, $j=1, ..., 6$. 
