...
Ideally, charts not only represent the data accurately, but  also allow readers to draw accurate conclusions. Generally, the human visual system is quite good at accurately interpreting charts \cite{cleveland:1984, kosara:2010}, but we need to be aware of contextual misperceptions that lead us to the wrong conclusion. While there are relatively few examples of the effect of optical illusions and other misperceptions on information graphics, \citet{amer2005bias} and \citet{poulton1985geometric} have documented the effect of the Poggendorff illusion on line graphs in different contexts. 
Tufte's lie factor does not give us any insight about the accuracy of the transition between chart and brain, but a similar factor could be created to measure the accuracy of this transition.
In this paper, we examine a situation in which low-level human perceptual processes interfere with making accurate judgments from displays and suggest an experimental methodology for estimating the psychological ``lie factor" of a chart due to a specific conceptual misperception: the sine illusion. 
...
\caption{The Necker Cube is a so-called ``ambiguous object" because two different transparent objects  produce the same retinal image (and thus the same perceptual experience). Commonly, the image seems to transition instantaneously from one possible mental representation to the other.}\label{fig:Necker} 
\end{figure}
...
\newdo{I think that I have finally figured out what my problem with $D_k$ is -- the psychological lie factor is defined as the ratio of the size of the perceived effect  and the size of the shown effect. That means that we cannot measure the psychological lie factor without input from somebody who perceives effects. Therefore we should not call $D_k$ the psychological lie factor of plot $k$. These values are all candidates until somebody picks one of the sub-plots. I think, it's just a naming convention. 
}

\newtext{In an attempt to estimate the psychological lie factor for each of the three functions, we first define the {\it distortion} $D_k$ for each sub plot $k=1, ..., 6$, as the ratio of the maximum line length to the minimum line length shown in the plot.
...
\newtext{One of the stimuli shown to participants is provided in figure \ref{fig:sinestimuli}. A participant might select plot 2 as appearing the most even; we compute the distortion factor $D_2 = 1.80/1.00 = 1.80$, that is, this participant identified a figure with lines varying in length from 1 to 1.8 as appearing to be the most consistent (weight values and the corresponding distortion factors are shown in Tables \ref{tab:w}, \ref{tab:w2}, and \ref{tab:w3} for sine, exponential, and inverse stimuli respectively. }
...
\newdo{I'm just checking on the demographics - we usually collect them in a separate table.
% 
% Let's trim down this section to the bare essentials: collecting data through amazon turk. Using filters to ensure good data quality, resulting in XXX responses from YYY participants. No discussions about excluding any responses. 
}
