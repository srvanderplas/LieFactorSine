...
\begin{table}[ht]
\centering
\tbl{\label{tab:w2} Ordered weight factors and corresponding lie factors for the exponential curve stimuli sets, as computed using equation \ref{eqn:psychliefactorD}.
% \hh{XXX what was the underlying concept behind the choice of weights? $D$ needs to be defined before this table is referred to - at least give a reference to $D$s definition. Is $D$ really the same for all three stimuli?}
}{
     \begin{tabular}{llrrrrrrlrrrrrr}
     \hline
...
\begin{table}[ht]
\centering
\tbl{\label{tab:w3} Ordered weight factors and corresponding lie factors for the inverse curve stimuli sets, as computed using equation \ref{eqn:psychliefactorD}. 
}{
     \begin{tabular}{llrrrrrrlrrrrrr}
     \hline
     %\cline{3-8} \cline{10-15}\\ [-1.75ex]
