...
There were \Sexpr{length(unique(turkdata$ip.id))} participants who completed at least 10 trials each (providing enough data that we could accurately fit individual-level parameters), and those participants completed \Sexpr{nrow(turkdata)} trials which were used for this analysis. 
