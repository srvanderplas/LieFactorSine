...
\caption{Another chart presented to participants through Amazon Mechanical Turk. This chart corresponds to set \#4 in table~\ref{tab:w3}. The plots are shown in random order, however; plot~\#1 corresponds to $w=0.7$, plot~\#2 to $w=0.1$, plot~\#3 to $w=0.5$, plot~\#4 to $w=1.1$, plot~\#5 to $w=0.3$, and plot~\#6 to $w=0.9$.}\label{fig:invstimuli}
\end{figure}
