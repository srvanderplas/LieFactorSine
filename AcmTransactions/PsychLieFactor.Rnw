...
\newtext{Tufte's lie factor does not give us any insight about the accuracy of the transition between chart and brain, but a similar factor could be created to measure the accuracy of this transition.}
In this paper, we examine a situation in which low-level human perceptual processes interfere with making accurate judgements from displays and suggest an experimental methodology for estimating the psychological ``lie factor" of a chart due to a specific conceptual misperception: the sine illusion. 

\subsection{The Curse of Three Dimensions}
% \comment{I don't understand how impossible objects are objects that have infinitely many two-dimensional representations -- isn't the idea of an impossible object that there is no 3d representation?}
