...
\newtext{
Ideally, charts not only represent the data accurately, but  also allow readers to draw accurate conclusions from the graph. While the human visual system is quite good at accurately interpreting statistical charts \cite{cleveland:1984}, we need to be aware of contextual misperceptions that lead us to the wrong conclusion. 
}
...
\comment{let's not undermine the whole idea of graphics in the intro ... in most situations our visual system is pretty good in interpreting graphics ... maybe cite Cleveland ordering of visual tasks, that got replicated by Kosara et al. -- \textbf{need to know more here, I can't find a similar paper on Google Scholar.}}

In this paper, we examine situations in which low-level human perceptual processes interfere with making accurate judgements from displays and suggest an experimental methodology for estimating the psychological ``lie factor" of a chart due to one of the identified conceptual misperceptions. 
...
\newtext{There are relatively few examples of the effect of optical illusions and other misperceptions on statistical graphics, but \citet{amer2005bias} and \citet{poulton1985geometric} have documented the effect of the poggendorff illusion on line graphs in different contexts. }
