...
\begin{figure}\centering
\includegraphics[keepaspectratio=TRUE,width=5in,trim=0in .5in 0in .5in]{figure/fig-sinedemo}
...
# 
# linedata <- data.frame(x=c(0, 0, 2*pi, 2*pi), y=c(3.5, 0, 3.5, 0), z=seq(-.5, .5, 1))
# xline <- linedata$x
# yline <- linedata$y
# zline <- linedata$z
...
text(trans3d(x=pi, y=max(yline)-.25, z=0, p2), label="Infinite Vanishing Point")
@
