...
\newtext{In an attempt to estimate the psychological lie factor for each of the three functions, we first define $D_k$ for each sub plot $k=1, ..., 6$, as the ratio of the maximum line length to the minimum line length shown in the plot.
