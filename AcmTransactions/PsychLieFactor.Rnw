...
\begin{table}[ht]
\centering
\tbl{\label{tab:w} Distortion factors used for the sine curve stimulus.}{
...
\caption{Uncorrected, optimally corrected (according to the mean value of $\theta$), and fully corrected stimuli for all three underlying functions used in the experiment.}\label{fig:allCorrectionOptimal}
\end{figure}
...
data$label <- paste("P = ", round(data$P, 2), data$paren, sep="")
