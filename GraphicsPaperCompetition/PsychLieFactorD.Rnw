...
<<setup, fig.keep='all', cache=FALSE, echo=FALSE, eval=TRUE, warning=FALSE, message=FALSE, error=FALSE>>=
...
source("./code/BayesAnalysisD.R") # generate pics for analysis without all the ddplying....
@
...
\includegraphics[width=\linewidth]{figure/fig-OverallMeansD}
...
\begin{figure}\centering
\includegraphics[width=\linewidth]{figure/fig-IndivMeanAllFcnsD}
...
\begin{figure}\centering
\includegraphics[width=\linewidth]{figure/fig-CIindivMeanD}
...
\begin{figure}\centering
\includegraphics[width=\linewidth]{figure/fig-spaghettiIndivDistsD}
...
Using the mean estimate of $\theta$ from the posterior distribution of each set of stimuli, we created for the sake of comparison plots with $w=0$, $w=1$, and $w=f(\theta)$. 
\begin{figure*}\centering
\includegraphics[width=\linewidth]{figure/fig-sineOptimalWeights}
\caption{Sine stimuli with $w=0$, optimally corrected (according to the mean value of $\theta$), and with $w=1$}\label{fig:sineCorrectionOptimal}
\end{figure*}
\begin{figure*}\centering
\includegraphics[width=\linewidth]{figure/fig-expOptimalWeights}
\caption{Exponential stimuli with $w=0$, optimally corrected (according to the mean value of $\theta$), and with $w=1$}\label{fig:expCorrectionOptimal}
\end{figure*}
\begin{figure*}\centering
\includegraphics[width=\linewidth]{figure/fig-invOptimalWeights}
\caption{Inverse stimuli with $w=0$, optimally corrected (according to the mean value of $\theta$), and with $w=1$}\label{fig:invCorrectionOptimal}
\end{figure*}
...
overall.mean.bounds
keys <- unique(data.frame(lie=key$d, w=key$w, test=key$test_param))
