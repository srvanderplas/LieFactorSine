...
Using the mean estimate of $theta$ from the posterior distribution of each set of stimuli, we created for the sake of comparison plots with $w=0$, $w=1$, and $w=f(\theta)$. 
\begin{figure*}\centering
